...
export class InternQualificationService {
    public static async addQualifications(
        ...
        const checkExistsQualification = await InternQualification.findOne({
            Intern: userId,
        });
        console.log(checkExistsQualification);
        if (checkExistsQualification !== null) {
            throw new AppError(
                "Qualification Has Been Already Added, Please Edit It",
                400
            );
        }
